...
void Graph::InsertEdge(const int& vertex1, const int& vertex2, int weight)
{
	...
	if (GetVertPos(vertex1) != (-1) && GetVertPos(vertex2) != (-1))
	{
		...
		{
			cout << "ERROR! Path length alredy exists" << endl;
			return;
		}
		else
		{
	...
	{
		cout << "ERROR! There are no such nodes " << endl;
		return;
	}
}
...
void Graph::eraseEdge(const int& vertex1, const int& vertex2)
{
	...
	{
		cout << "ERROR! Graph does not have one or these vertices " << endl;
		return;
	}
}
...
void makeGraph()
{
	...
	cout << "Сколько узлов добавить: "; cin >> amountVerts;
	cout << "Сколько связей добавить: "; cin >> amountEdges;
	...
	{
		cout << "Откуда: "; cin >> sourceVertex;
		cout << "Куда: "; cin >> targetVertex;
...
void mouseClick(int button, int state, int x, int y)
{
	...
	if (x >= 50 && x <= WinW / 7 && y <= 4 * (WinH / 7) && y >= 3 * (WinH / 7) + 20)
	{
		if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN)
		{
			...
			cout << "How much to add: "; cin >> vertNum;
